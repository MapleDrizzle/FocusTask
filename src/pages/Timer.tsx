...
export default function Timer() {
  ...
  const moodColor = (m: Mood) =>
    m === "High" ? "#2e7d52" : m === "Low" ? "#c0392b" : "#b07d1a";
    ...
    <main className="timer-layout">
      ...
      <div className="timer-top-cards">
        ...
        <div className="timer-stat-card">
          ...
          <p>{sessions.filter((s) => s.completed && !s.isTest).length}</p>
        </div>
      </div>

      {/* Main timer card */}
      <div className="timer-main-card">
        <TimerDisplay totalSeconds={secondsLeft ?? 0} />

        {/* Progress bar */}
        {isRunning && showProgressBar && (
        ...
        {/* Mood selector */}
        {!isRunning && (
          <div className="mood-row" role="group" aria-label="Focus mood">
            <span className="mood-label">Focus mood:</span>
        ...
        <div className="timer-actions">
          {!isRunning ? (
            <div className="start-btn-group">
              ...
              <button className="break-button" onClick={startBreakTimer}>
                Start 15 Min Break
              </button>
            </div>
          ) : (
      ...
      <section className="sessions-section">
        <h3 className="section-heading">Previous Sessions</h3>
              ...
              <div key={s.id} className="session-row">
                ...
                <div className="session-info">
                  <span className="session-name">
                    ...
                    {s.isTest && <span className="test-tag">test</span>}
                  </span>
                  ...
                  <span className="session-meta">
                    ...
                    {s.completed ? (
                      ...
                      <span className="status-stopped">stopped early</span>
                    )}{" "}
                    · mood: {s.mood}
                  </span>
                </div>

                <span className="session-duration">{s.minutes} min</span>
              </div>
            ))
          )}
        </div>
